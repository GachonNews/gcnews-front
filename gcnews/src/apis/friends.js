...
export const fetchMyProfile = async () => {
  try {
    ...
    console.log(
      "fetchMyProfile: /api/user-info/profile 응답 res 객체:",
    ...
    if (
      ...
      res.data &&
      res.data.data &&
      typeof res.data.data.name !== "undefined"
    ) {
      ...
      console.error(
        "fetchMyProfile: API 응답 구조 문제 또는 name 없음. res.data:",
        JSON.stringify(res.data, null, 2)
      );
      return {
        name: undefined,
        id: undefined,
        error: "Invalid API response for my profile",
      };
    }
  } catch (error) {
    console.error(
      "fetchMyProfile: API 호출 중 오류:",
...
export const fetchFriendIdList = async () => {
  try {
    ...
    console.log(
      "fetchFriendIdList: /api/user-info/friend 응답 res.data.data:",
      JSON.stringify(res.data.data, null, 2)
    );
    return res.data.data || []; // API가 data 필드에 배열을 반환한다고 가정
  } catch (error) {
    console.error(
      "fetchFriendIdList: API 호출 중 오류:",
      error.response ? error.response.data : error.message
    );
    return [];
  }
...
export const fetchFriendNameById = async (friendId) => {
  ...
  try {
    console.log(
      `fetchFriendNameById: 친구 ID "${friendId}"로 이름 조회 시작, URL: ${apiUrl}`
    );
    const res = await axiosInstance.get(apiUrl); // 쿼리 파라미터 방식이면 두 번째 인자로 { params: { userId: friendId } } 전달
    console.log(
      `fetchFriendNameById: ID "${friendId}" 응답 res 객체:`,
      JSON.stringify(res, null, 2)
    );
    ...
    if (res && res.data && res.data.data) {
      ...
      if (
        typeof friendProfile.name === "string" &&
        ...
        console.warn(
          `fetchFriendNameById: ID "${friendId}"의 이름이 null이거나 유효하지 않습니다. 응답 데이터:`,
          JSON.stringify(friendProfile, null, 2)
        );
        return `친구(${friendId})`; // 기본값
      }
    } else {
      console.warn(
        `fetchFriendNameById: ID "${friendId}"에 대한 응답 데이터 구조가 예상과 다릅니다. res.data:`,
        JSON.stringify(res.data, null, 2)
      );
    ...
    console.error(
      `fetchFriendNameById: ID "${friendId}" 이름 조회 중 오류 발생:`,
      error.response ? error.response.data : error.message
    );
...
export const fetchMonthlyActivities = async (userId, year, month, isMe) => {
  ...
  try {
    console.log(
      `fetchMonthlyActivities: API 호출 시작 - URL: ${apiUrl}, Params:`,
      params,
      `isMe: ${isMe}`
    );
    const res = await axiosInstance.get(apiUrl, { params });
    console.log(
      `fetchMonthlyActivities: 응답 (${apiUrl}):`,
      JSON.stringify(res.data, null, 2)
    );
    ...
    console.error(
      `fetchMonthlyActivities: API 호출 오류 (${apiUrl}):`,
      error.response ? error.response.data : error.message
    );
    return []; // 오류 시 빈 배열 반환
  }
...
export const fetchFriendDetailsById = async (friendId) => {
  ...
  try {
    ...
    console.log(
      `[fetchFriendDetailsById] friendId ${friendId} 응답 데이터:`,
      JSON.stringify(
        {
          status: res.status,
