...
// 홈 아이콘 SVG (간단한 예시)
const HomeIcon = (
  { size = 28, color = "#333" } // 아이콘 색상 기본값 설정
) => (
  <svg
    width={size}
    height={size}
...
const CalendarLayout = () => {
  ...
  useEffect(() => {
    const loadFriends = async () => {
      try {
        console.log(
          "CalendarLayout: loadFriends - 내 정보 및 친구 목록 로딩 시작"
        );
        ...
        if (meData && meData.name && meData.id && !meData.error) {
          setMyProfile(meData);
          console.log(
            "CalendarLayout: loadFriends - 내 정보 (meData 객체):",
            JSON.stringify(meData, null, 2)
          );

          const idListFromApi = await fetchFriendIdList(); // 예: [{ userId, friendId }, ...]
          console.log(
            "CalendarLayout: loadFriends - fetchFriendIdList 결과:",
          ...
          console.log(
            "CalendarLayout: loadFriends - 최종 friendList (객체배열):",
            JSON.stringify(processedFriendListObjects, null, 2)
          );
        } else {
          console.error(
            "CalendarLayout: loadFriends - 내 프로필 정보 로딩 실패 또는 오류. meData:",
            JSON.stringify(meData, null, 2)
          );
  ...
  useEffect(() => {
    const loadActivitiesForCalendar = async () => {
      if (myProfile && selectedFriend && year && month) {
        ...
        console.log(
          `CalendarLayout: loadActivities - 사용자 ID '${currentUserId}' (${
        ...
        console.log(
          "CalendarLayout: Fetched Monthly API Data (Array):",
        ...
        if (monthlyApiActivityArray && Array.isArray(monthlyApiActivityArray)) {
          monthlyApiActivityArray.forEach((activityLog) => {
            ...
            if (!dateKey || typeof dateKey !== "string") {
              console.warn(
                "CalendarLayout: 활동 로그에 유효한 visitDate가 없습니다:",
                activityLog
              );
              return;
            }
        ...
        console.log(
          `CalendarLayout: 사용자 ID '${currentUserId}' 활동 데이터 가공 완료:`,
